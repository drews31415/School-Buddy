...
class MonitoringStack extends cdk.Stack {
    constructor(scope, id, props) {
        ...
        // ──────────────────────────────────────────────────────
        const crawlerErrors = application.crawlerFn.metricErrors({ period: cdk.Duration.minutes(5), statistic: "Sum" });
        const crawlerInvocations = application.crawlerFn.metricInvocations({ period: cdk.Duration.minutes(5), statistic: "Sum" });
        ...
        const w1Errors = lambdaFunctions.map(({ fn, label }) => fn.metricErrors({ label: `${label} 에러`, period: cdk.Duration.minutes(5) }));
        const w1Invocations = lambdaFunctions.map(({ fn, label }) => fn.metricInvocations({ label: `${label} 호출`, period: cdk.Duration.minutes(5) }));
        ...
        new aws_cdk_lib_1.aws_xray.CfnSamplingRule(this, "SamplingRule", {
            samplingRule: {
